...
var MessageBoard = (function () {
    function MessageBoard(jQuery, options) {
        ...
        defaultOptions.currentUserID = 1;
        ...
        this.$textBox = $("<textarea />").attr("rows", "1").attr("placeholder", "write message").addClass("chat-window-text-box").appendTo($windowTextBoxWrapper);
    ...
    MessageBoard.prototype.addMessage = function (message, clientGuid, scroll) {
        ...
        if (message.ClientGuid && $("p[data-val-client-guid='" + message.ClientGuid + "']").length) {
            ...
            if ($lastMessage.length && $lastMessage.attr("data-val-user-from") == message.UserFromId.toString()) {
				...
				var UserClass = '';
				if (message.UserFromId == this.options.currentUserID) {
                ...
                var $chatMessage = $("<div/>").addClass("chat-message").addClass(UserClass).attr("data-val-user-from", message.UserFromId);
